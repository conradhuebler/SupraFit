...
- 2026-07-14: ✅ **Thermogram per-injection volume editable** — column 0 of the import dialog table (`thermogram.cpp`) accepts manual edits, persisted to `m_inject` (guarded by `m_updating_table`; heat columns read-only). `ThermogramWidget` scaling relabelled "cal → J" with a tooltip (leaving it at 1 for cal data is why results match calorie tools).
