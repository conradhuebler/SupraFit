...
#include "uv_vis_2_1_1_1_1_2_Model.h"

uv_vis_IItoI_ItoI_ItoII_Model::uv_vis_IItoI_ItoI_ItoII_Model(DataClass* data)
...
uv_vis_IItoI_ItoI_ItoII_Model::uv_vis_IItoI_ItoI_ItoII_Model(AbstractTitrationModel* data)
    : AbstractTitrationModel(data)
{
    m_threadpool = new QThreadPool(this);
    for (int i = 0; i < DataPoints(); ++i)
...
uv_vis_IItoI_ItoI_ItoII_Model::~uv_vis_IItoI_ItoI_ItoII_Model()
{
    qDeleteAll(m_solvers);
    qDebug() << "time for calculation" << m_time << "in" << Description();
}

void uv_vis_IItoI_ItoI_ItoII_Model::DeclareOptions()
{
...
void uv_vis_IItoI_ItoI_ItoII_Model::EvaluateOptions()
{
...
void uv_vis_IItoI_ItoI_ItoII_Model::InitialGuess_Private()
{
    qreal factor = 1;
...
QVector<qreal> uv_vis_IItoI_ItoI_ItoII_Model::DeCompose(int datapoint, int series) const
{
...
QSharedPointer<AbstractModel> uv_vis_IItoI_ItoI_ItoII_Model::Clone(bool statistics)
{
    QSharedPointer<uv_vis_IItoI_ItoI_ItoII_Model> model = QSharedPointer<uv_vis_IItoI_ItoI_ItoII_Model>(new uv_vis_IItoI_ItoI_ItoII_Model(this), &QObject::deleteLater);
...
MassResults uv_vis_IItoI_ItoI_ItoII_Model::MassBalance(qreal A, qreal B)
{
    //     QMutexLocker (&mutex);
    MassResults result;
...
QString uv_vis_IItoI_ItoI_ItoII_Model::AdditionalOutput() const
{
    QString result;
    /*
    // double max = 1e3;
