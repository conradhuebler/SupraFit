/*
 * SupraFit - flexible UV/Vis titration model (arbitrary N-component equilibrium via BFGS speciation)
...
{
    // seed the free-component extinctions from the first spectrum, the complexes from the last
...
void uvvis_any_Model::CalculateConcentrations()
{
    ...
    // design matrix for Beer-Lambert: absolute concentrations of free components then species
...
{
    CalculateConcentrations();
    // Fit the molar extinction coefficients (local parameters) linearly to the spectra given the
    // absolute species concentrations (Beer-Lambert design matrix).
...
{
    CalculateConcentrations();
    // Absorbance = sum over free components and species of concentration * extinction coefficient.
