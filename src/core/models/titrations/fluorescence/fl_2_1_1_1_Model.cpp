...
fl_IItoI_ItoI_Model::fl_IItoI_ItoI_Model(AbstractTitrationModel* data)
    : AbstractTitrationModel(data)
{
    PrepareParameter(GlobalParameterSize(), LocalParameterSize());
}
        ...
        for (int j = 0; j < SeriesCount(); ++j) {
            if (i == 0) {
                ...
                value = (host * LocalTable()->data(j, 1))
                    + complex_21 * LocalTable()->data(j, 2)
                    + complex_11 * LocalTable()->data(j, 3);
            }
...
QVector<qreal> fl_IItoI_ItoI_Model::DeCompose(int datapoint, int series) const
{
...
QSharedPointer<AbstractModel> fl_IItoI_ItoI_Model::Clone(bool statistics)
{
