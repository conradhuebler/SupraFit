...
ItoI_ItoII_Model::ItoI_ItoII_Model(AbstractTitrationModel* data)
    : AbstractTitrationModel(data)
{
    PrepareParameter(GlobalParameterSize(), LocalParameterSize());
}
...
void ItoI_ItoII_Model::OptimizeParameters_Private()
{
...
QVector<qreal> ItoI_ItoII_Model::DeCompose(int datapoint, int series) const
{
    QString method = getOption(Method);
...
QSharedPointer<AbstractModel> ItoI_ItoII_Model::Clone(bool statistics)
{
...
QString ItoI_ItoII_Model::ParameterComment(int parameter) const
{
    if (parameter == 0)
...
QString ItoI_ItoII_Model::ModelInfo() const
{
...
QString ItoI_ItoII_Model::AnalyseGridSearch(const QJsonObject& object, bool forceAll) const
{
