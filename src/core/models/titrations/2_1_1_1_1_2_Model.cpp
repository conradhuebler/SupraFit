...
IItoI_ItoI_ItoII_Model::IItoI_ItoI_ItoII_Model(AbstractTitrationModel* data)
    : AbstractTitrationModel(data)
{
    m_threadpool = new QThreadPool(this);
    for (int i = 0; i < DataPoints(); ++i)
...
void IItoI_ItoI_ItoII_Model::OptimizeParameters_Private()
{
    QString coop21 = getOption(Cooperativity2_1);
    QString coop12 = getOption(Cooperativity1_2);
...
QString IItoI_ItoI_ItoII_Model::ParameterComment(int parameter) const
{
    if (parameter == 0)
