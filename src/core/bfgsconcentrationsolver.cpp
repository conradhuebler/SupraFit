...
std::vector<double> BFGSConcentrationSolver::solve()
{
    ...
    // Levenberg-Marquardt damping: solve (H + lambda*I) d = -g. lambda self-tunes per system and per
    // iteration -> shrinks toward 0 (full Newton, quadratic convergence) when steps succeed, grows
    // (small, gradient-like, guaranteed-descent steps) when the Hessian is ill-conditioned or we are
    // far from the solution. This is robust across weak/strong/competitive binding without any
